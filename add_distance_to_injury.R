...
if(file.exists(paste0(overflow_path,'processed_injuries_8.Rds'))){
  ...
  ## distance data
  # scale by year
  ##!! needs to match a sheet called scalingweight_cityreg which on 28/12/2019 is sheet number 4
  scale_by_year_raw <- readxl::read_xls('../mh-scenarios/1_InputData/2_CityRegion_scaling/RTS_NTS_distances_cityreg.xls',sheet='scalingweight_cityreg',col_names=T)
...
## save for prediction and compute model #############################################
...
for(i in 1:2)
  for(j in 1:2)
    city_table[[i]][[j]] <- injury_table[[i]][[j]][injury_table[[i]][[j]]$year==pred_year,]
...
basic_formula <- 'count~year+cas_severity+cas_mode*strike_mode+road+road:(cas_mode+strike_mode)+region+cas_mode:cas_severity+strike_mode:cas_severity'
...
for(i in 1:2) {
  ...
  for(j in 1:2) {
    ...
    city_table[[i]][[j]]$base_pred <- predict(trimmed_mod,newdata=city_table[[i]][[j]],type='response')
